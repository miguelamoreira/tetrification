...
export default {
  ...
  methods: {
    ...
    handleBtnClick(type) {
      ...
      if (type === 'past') {
        btnPast.classList.add('btn-active');
        btnFuture.classList.remove('btn-active');
        this.searchQuery = '';
        this.visibleGames = this.getPastGames();
      } else if (type === 'future') {
        btnFuture.classList.add('btn-active');
        btnPast.classList.remove('btn-active');
        this.searchQuery = '';
        this.visibleGames = this.getFutureGames();
      } else {
        btnPast.classList.remove('btn-active');
        btnFuture.classList.remove('btn-active');
        this.visibleGames = [];
    ...
    getLiveGames() {
      const liveGameIds = [5, 6];
      console.log('All games:', this.gameStore.getGames);
      
      const liveGames = liveGameIds.map(gameId => {
        const gamesId = this.gameStore.getGames.filter(game => game.gameId === gameId);
    ...
    getPastGames() {
      const pastGameIds = [1, 2, 3, 4];
      console.log('All games:', this.gameStore.getGames);
      
      const pastGames = pastGameIds.map(gameId => {
        const gamesId = this.gameStore.getGames.filter(game => game.gameId === gameId);
    ...
    getFutureGames() {
      const futureGameIds = [7];
      console.log('All games:', this.gameStore.getGames);
      
      const futureGames = futureGameIds.map(gameId => {
        const gamesId = this.gameStore.getGames.filter(game => game.gameId === gameId);
        return gamesId.length > 0 ? gamesId[0] : null;
      }).filter(game => game !== null);
    ...
    searchGames() {
      const searchPlayer = this.searchQuery.toLowerCase().trim();
      ...
      this.visibleGames = this.games().filter(game => {
        const player1 = game.player1.toLowerCase().includes(searchPlayer);
        const player2 = game.player2.toLowerCase().includes(searchPlayer);

        if ([1, 2, 3, 4].includes(game.gameId)) {
          // Retrieve the last object of the array for gameId 1-4
          const filteredGames = this.gameStore.getGames.filter(g => g.gameId === game.gameId);
          return player1 || player2 && filteredGames.length > 0 ? game === filteredGames[filteredGames.length - 1] : false;
        } else if ([5, 6, 7].includes(game.gameId)) {
          // Retrieve the first object of the array for gameId 5-7
          const filteredGames = this.gameStore.getGames.filter(g => g.gameId === game.gameId);
          return player1 || player2 && filteredGames.length > 0 ? game === filteredGames[0] : false;
        }
  ...
  created() {
    if (this.games().length === 0) {
      this.gameStore.fetchGames().then(() => {
